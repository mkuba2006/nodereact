...
const User = ({ user, hoveredUserId, handleDeleteUser, handleMouseEnter, handleMouseLeave }) => {
  return (
    <li key={user.id}>
      ...
      <p id='data'>
        <div id='ids'><b>{user.id}</b></div>|{user.Name}: {user.Password} </p>
      <Tooltip label='Delete' placement='right'>
        <button id='delete' onClick={() => handleDeleteUser(user.id)} onMouseEnter={() => handleMouseEnter(user.id)} onMouseLeave={handleMouseLeave}>
